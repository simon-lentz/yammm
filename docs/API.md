...
A snapshot holding a shape the writers cannot render is refused with an error matching `json.ErrUnrepresentable`, so a caller separates it from an encoding failure and from an I/O failure without matching the message text. One shape reaches them: an edge whose target key has a different number of components from the target type's declared key, which `graph.RebuildSnapshot` does not check — it validates identity, denoted types, root types and cardinality, and no key arity — so a `.ys` document can carry one. The writers' other refusal arm, an edge target type that does not resolve, is not data a caller can act on: `RebuildSnapshot` refuses a type identity the schema cannot resolve at every position its parts carry one, so reaching it means a snapshot built against another schema was imported, against the contract `graph.NewFromSnapshot` and `graph.NewBatchAssemblerFromSnapshot` state.
