...
1. **`Registry.Register` is idempotent for the same source.** Registering the same `SourceID` twice is a no-op when the entry's bytes match (both schemas carry their sources) and the `StructuralHash` matches — the first pointer remains stored and type-index entries are not re-indexed. A schema with no sources (Builder-built) is compared by hash alone. A source whose bytes or hash changed returns `*RegistryError{Kind: DuplicateSourceID}` naming what differed; the hash alone would call an annotation or documentation edit identical, since it excludes both by design.
