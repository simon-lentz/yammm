...
| `Enum` (inline) | `string` underlying | **yes** — `type <Type><Field> string` + value consts; on an edge property the owner is the `EDGE_` struct (`EDGE_Person_works_at_CompanyRole`) |
...
- **Optional `List`/`Vector`** → the slice stays nilable (no extra pointer) + `,omitzero`: a nil slice is left out and a present empty list is written as `[]`, because the library keeps an empty list apart from an absent one and `,omitempty` would drop it.
