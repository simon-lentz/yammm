...
`DuplicateParts` states the conflict's address (`ConflictType`, `ConflictKey`) beside the rejected instance's own identity, plus `ParentType` / `ParentKey` / `Relation` for a composed-child duplicate: a root conflict resolves through the instance index, a composed conflict through the parent's relation slot, and an empty stated key addresses the slot's sole occupant. `RebuildSnapshot` rejects a `schema.TypeID` that is zero, or that the schema cannot resolve, at any parts position with a Fatal `E_INTERNAL` — identity is total at this boundary, and every identity names a type of the schema. Most positions name both the position and the key; a zero entry in `Types` names its index, and a zero `Instances` map key names the group's size.
...
A snapshot holding a shape the writers cannot render is refused with an error matching `json.ErrUnrepresentable`, so a caller separates it from an encoding failure and from an I/O failure without matching the message text. `graph.RebuildSnapshot` reconstructs a document and does not validate one — it checks identity, denoted types, root types and composition cardinality — so a `.ys` document can carry shapes `Graph.Add` never builds: an edge whose target key has another arity than its target type's, a (one) association carrying several edges, an edge to a type its association does not declare, and an edge or composed children under a name the type declares no relation of that kind for. Each would be written as a document `ParseObject` and the validator refuse, so each is refused instead. The writers' other refusal arm, an edge target type that does not resolve, is not data a caller can act on: `RebuildSnapshot` refuses a type identity the schema cannot resolve at every position its parts carry one, so reaching it means a snapshot built against another schema was imported, against the contract `graph.NewFromSnapshot` and `graph.NewBatchAssemblerFromSnapshot` state.
