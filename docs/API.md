...
`NewFromSnapshot` imports all instances, edges, duplicates, and unresolved records from the source snapshot. Subsequent `Add` calls may resolve previously-unresolved edges if they supply the missing targets. A snapshot built against another schema is held to the importing one under the graph package doc's "Structural facts": one that breaks a fact is refused with a nil `Graph`, under the code `Graph.Add` refuses it with, and nothing is installed. Keys are rewritten under the importing schema's key constraints, and whether each unresolved record's association is required is read from it.
...
`UnresolvedParts.Properties` carries DSL-declared edge property values from the forward reference and is populated only when `Reason` is `"target_missing"` — `"absent"` and `"empty"` describe a missing/empty reference that never had a target. No field states whether the association is required: `RebuildSnapshot` reads it from the schema, as `Graph.Add` does. The wire persists the values through Marshal/Load, symmetric with resolved `Edge.Properties`. See the snapshot package's [Wire Format Versions](#wire-format-versions) subsection for the current accept range.

`DuplicateParts` states the conflict's address (`ConflictType`, `ConflictKey`) beside the rejected instance's own identity, plus `ParentType` / `ParentKey` / `Relation` for a composed-child duplicate: a root conflict resolves through the instance index, a composed conflict through the parent's relation slot, and an empty stated key addresses the slot's sole occupant. `RebuildSnapshot` holds the parts to the graph package doc's "Structural facts". It rejects a `schema.TypeID` that is zero, or that the schema cannot resolve, at any parts position with a Fatal `E_INTERNAL` naming the identity — identity is total at this boundary, and every identity names a type of the schema — and so it rejects a denoted or root type that cannot hold a root, an undeclared property name, two roots of one type at one canonical address, an unresolved record with an undocumented reason or an `absent` or `empty` record carrying a target key or edge properties, and a duplicate record whose type or key is not its instance's. A composition slot, a key and an association record draw the code `Graph.Add` refuses them with. Most positions name both the position and the key; a zero entry in `Types` names its index. It panics on a nil schema.
