...
Every type name whose value is an array is an entry of the returned map, an empty array included, unless it is read after a fault that stops the parse; so a caller that validates each entry reaches every type name whose array was read, and a name the schema does not declare draws `E_INSTANCE_TYPE_NOT_FOUND` from `instance.Validator` whatever its array holds. Three shapes a JSON decoder would resolve silently are Error `E_ADAPTER_PARSE` diagnostics instead, and each is reported without dropping anything the document states, because what a decoder keeps of them is one reader's convention and not the document's meaning. A type name repeated as a root key is reported at the repeat, and both arrays' instances are in the entry, each indexing its own array, where decoding keeps the last array and drops the first batch whole. A member name repeated inside one object, at any depth of an instance, is reported at each repeat, and the instance is produced holding the value the decoder kept — the value every other JSON reader keeps. Names compare as the decoder reads them: escapes resolved, and each byte of an invalid UTF-8 sequence read as U+FFFD. Anything but white space and closed comments after the root object is reported where it starts, whether or not it reads as a JSON value; an unterminated block comment is content. And a fault the decoder cannot read past — a syntax error, a truncated document — is reported and parsing stops there, returning every instance read before it and reading nothing after it.
