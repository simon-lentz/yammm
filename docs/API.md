...
Cross-schema imports are closure-flattened into one self-contained document with gogen's collision rule (bare names where unique, `<schemaName>.<Name>` on collision, hard error when qualification cannot separate two entities of one schema). A dotted key never equals a bare one, so the numeric suffix gogen gives a taken qualified Go name has no counterpart here. Abstract types get no `$defs` entry (nothing can legally `$ref` one); their members appear flattened into each subtype.

Fidelity caveats. The emitted schema does not reproduce yammm's validation, and each divergence changes only what the editor flags. `yammm check` runs the parse and instance validation; graph assembly runs in `yammm load`, `yammm export` and `yammm snapshot save`, and `yammm check` does not run it.
...
- **Numbers** — JSON Schema reads a number exactly; yammm reads a plain integer literal that fits as an `int64` and any other number as a `float64`. The editor accepts a number yammm cannot hold (`1e400` on a Float, `9223372036854775808` on an Integer) and yammm refuses it (`E_TYPE_MISMATCH`); a decimal or exponent literal on an Integer is read through `float64`, so the two can disagree in both directions near that precision's limits.
...
- **Patterns and layouts** — patterns pass through verbatim (yammm compiles RE2, JSON Schema validators assume ECMA-262); a custom `Timestamp["layout"]` emits a plain string with the source form in its `description`.
