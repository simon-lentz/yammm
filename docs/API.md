...
**A type is named the way the entry schema names it** — in its heading, every link to it, the `from <Owner>` markers and the diagram: the bare name for a type the entry declares, the entry's own tag `<alias>.<TypeName>` for one it imports directly (the tag `schema.AddressableTag` returns, which is how a data file names it), and `<TypeName> (<schemaName>)` for one it reaches only through another import, which has no tag. **Every heading the generator writes takes the anchor GitHub gives it**: its slug, suffixed `-1`, `-2`, … when an earlier heading took it, so two such headings that slug alike each keep a working link, and a type named `Types` links to its own heading rather than the `## Types` section. The generator allocates anchors without reading doc comments, and a heading inside one takes an anchor on GitHub too: it can move a later heading's suffix, and when it takes a type's anchor first, a link to that type lands on the doc comment's heading.

**Doc-comment text is written as the Markdown its author wrote**: a link, table or heading inside it is the author's own. A code fence a doc comment leaves open, as CommonMark's fence rules read it, is closed at the end of that comment's block, and the generator's own fences are sized past any backtick run in their body. The seal reads fences alone: a fence inside an HTML block or inside a list the author wrote is read as if it stood at the top level, so such a doc comment can still leave the document inside a fence. Every other text the schema supplies renders literally. A code cell — a property's name and Type, a data type's name and Definition — shows its text byte for byte: a code span with each pipe escaped, or, for text holding a backtick, a backslash before a pipe or a line break, a `<code>` element whose Markdown-significant characters are entities. A description cell escapes backslashes and pipes and folds newlines to `<br>`. A schema name and an invariant message are escaped for Markdown, a control character in a schema name is written as its Go escape (`\n`), and a double quote in a Mermaid class label is written `#quot;`.

### Validation

Output is deterministic (byte-identical across runs and checkouts), so generated documents can be committed and drift-checked by regenerate-and-diff; the document is a versioned surface, whose tiers `docs/VERSIONING.md` states. Before returning, `Marshal` checks the structure it wrote itself: every fence closes, no heading it wrote sits inside an open fence, every internal link it wrote resolves to a heading of the document, and every table row it wrote has its header's column count on one line — a failure is a generator bug returned as an error, never emitted output. No schema is refused for its names or its doc-comment text.
