...
**The header and the records.** The header must name every column, once: a repeated or empty column name is refused at the header's line and no record is read, and a name is matched as written, never trimmed. A record the reader refuses — a field count that differs from the header's, a bare quote, a character after a closing quote — is an Error at the line the fault is on, and the parse goes on with the next record (a quoted field that is never closed runs to the end of the input); `LazyQuotes` is off, so a malformed quoted field is never read as a value. Any other error the reader returns — the `io.Reader` failing, an error that only wraps `io.EOF` included — stops the parse with a Fatal `E_ADAPTER_IO`, as the `diag` package documents for an I/O failure, and keeps the records read before it.

**The parser owns the CSV and the validator owns every name**, as for the JSON adapter, so a row parses as the JSON object holding the same keys does and the validator answers the two alike. Column names resolve by the validator's rule, applied to the object each row makes (each target of an edge group is its own object), at a property, an association's field, an edge property and a key component: a name that spells a member exactly claims it when the object holds it; otherwise a name whose `instance.FoldKey` matches the member resolves to it when it is the member's one such name the object holds, or its one candidate in the header. `WithStrictPropertyNames(true)` matches exactly instead, and must agree with the validator's setting: a folding parser under a strict validator writes a mis-cased column's empty value, so the row draws an unknown field. The resolved member decides coercion, the empty cell and edge assembly, and every key keeps the header's spelling. A name that resolves to no member — a plain column, an edge suffix, a key component the target does not declare, a dotted field naming no association — is carried as its text for the validator to report, or to ignore under `WithAllowUnknownFields(true)`; its empty cell is skipped. A cell that does not coerce draws `E_CSV_COERCE` and keeps its text. Two values for one key are refused, as a repeated JSON member is: a plain column and the dotted columns of the same field both holding a value draw `E_ADAPTER_PARSE`, the code that adapter reports its repeated member under, and the group is kept. Under `ParseWithTypeColumn` each row type reads the header against its own members, and a type-column value that is not a type name by the grammar — the rule the JSON adapter applies to a top-level key, the empty value included — draws `E_INVALID_TYPE_TAG` and its record is skipped.
...
An association renders as dotted columns (v0.15.0): one `<field>._target_<pk>` column per target key component and one `<field>.<prop>` column per declared edge property — the same `_target_` shape the JSON adapter and `instance.Validator` exchange. A `(many)` association zips its group across the list separator: segment `i` of every column in the group describes target `i`, and the segment counts must agree (`E_ADAPTER_PARSE` names the relation on a mismatch). A group is absent only when every cell in it is empty; inside a present group an empty cell stands for an empty segment on every target. Empty segments are read as Empty Cells states. Edge properties are scalars by language rule — a `List`-typed relation property draws `E_LIST_ON_EDGE` and a `Vector`-typed one draws `E_INVALID_CONSTRAINT` — which is what makes zipping well-founded.
