...
- **Every relation field** carries `,omitempty`, required ones included: the parser refuses a `null` relation, and a required relation left unset is refused where presence is checked — graph assembly for an association (`E_UNRESOLVED_REQUIRED`), instance validation for a composition (`E_UNRESOLVED_REQUIRED_COMPOSITION`).
