...
func isCleanShutdown(err error) bool {
	...
	// Fallback: check the error tree for wrapped EPIPE that doesn't satisfy errors.Is.
	// Some libraries wrap EPIPE in custom error types that break the chain.
	errMsg := err.Error()
	if strings.Contains(errMsg, "broken pipe") || strings.Contains(errMsg, "EPIPE") {
		return true
	}
	return false
}
...
func run(stdout io.Writer, args []string) error {
	...
	case sig := <-sigCh:
		...
		_ = server.Close() // ordered shutdown: workspace.Shutdown() + jrpc2.Stop(); Close always returns nil
		...
		if err := os.Stdin.Close(); err != nil {
			logger.Debug("error closing stdin", slog.Any("error", err))
		}
		...
		case err := <-errCh:
			if err != nil {
				...
				logger.Debug("RunStdio returned after close", slog.Any("error", err))
			}
