...
func TestSum_AllocatesNothingPerElement(t *testing.T) {
	...
	allocs := func(in any) float64 {
		return testing.AllocsPerRun(100, func() {
			if _, err := builtinSum(t.Context(), nil, in, nil, nil, nil, nil); err != nil {
				t.Fatal(err)
			}
		})
	}
	...
	if got, err := builtinSum(t.Context(), nil, mixedSmall, nil, nil, nil, nil); err != nil || got != 6.5 {
		t.Errorf("mixed sum = %v, %v; want 6.5", got, err)
	}
	huge := []any{int64(1 << 62), int64(1 << 62), 0.5}
	if got, err := builtinSum(t.Context(), nil, huge, nil, nil, nil, nil); err != nil {
	...
	if _, err := builtinSum(t.Context(), nil, []any{int64(1 << 62), int64(1 << 62)}, nil, nil, nil, nil); err == nil {
